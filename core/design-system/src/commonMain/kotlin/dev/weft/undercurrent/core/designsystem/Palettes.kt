...
/**
 * Compose color tables backing each [AppPalette]. The enum itself
 * (`:core:model`) stays Compose-free; the (dark, palette) →
 * [UndercurrentColors] mapping lives here.
 *
 * Use via the [colors] extension function on [AppPalette].
 *
...
fun AppPalette.colors(dark: Boolean): UndercurrentColors = when (this) {
    ...
    AppPalette.Vellum -> if (dark) VellumDark else VellumLight
}
...
// ─────────────────────────────────────────────────────────────────────────
// H — Vellum / sepia (no chromatic accent — accent is darker ink tone)
// ─────────────────────────────────────────────────────────────────────────
